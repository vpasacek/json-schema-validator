...
class json_schema_validator_base
{
...
class json_schema_validator_reference : public json_schema_validator_base
{
public:
};


class json_schema_validator_property
{
	std::string name_or_pattern;
	json_schema_validator_base *schema;
};


class json_schema_validator_object : public json_schema_validator_base
{
	std::vector<json_schema_validator_property> children;
};
...
int main(void)
{
	...
	//
	// ref to http://localhost:1234/folder/folderInteger.json  (via ID)
